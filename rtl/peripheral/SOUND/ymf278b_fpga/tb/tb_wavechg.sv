...
module tb_wavechg;
    ...
    always_ff @(posedge clk) begin
        ...
        // v3: the edge is consumed within the same slot turn (SL_ACC), so
        // sample it at the engine's own consume point instead of at advance.
        else if (dut.retrig_consume && dut.w_slot==5'd0 && dut.w_edge)
            s0_edge_seen <= 1'b1;
    end
    ...
    // Set a wave number (regs 0x20/0x08) and KEY ON after `gap` frames, WITHOUT
    // waiting for the header to load (mimics the BASIC tester FOR J=0TO20 delay).
    logic [21:0] hs; logic [15:0] hl, he; logic [1:0] hb; logic [3:0] har;
    task set_and_key(input [8:0] wv, input int gap, input string label);
        write_reg(8'h68, 8'h00);                 // key off
        write_reg(8'h20, {7'd0, wv[8]});         // wave high bit
        write_reg(8'h08, wv[7:0]);               // wave low byte
        repeat (gap) frames(1);
        write_reg(8'h68, 8'h80);                 // KEY ON (maybe before header ready)
        frames(2);
        $display("  [%s wv=%0d gap=%0d] hdr start=%h loop=%h end=%h bits=%b AR=%0d  hf_pend0=%b",
                 label, wv, gap, dbg_slot0_hdr_start, dbg_slot0_hdr_loop,
                 dbg_slot0_hdr_end, dbg_slot0_hdr_bits, dbg_slot0_ar, dbg_hf_pending[0]);
    endtask
    // Capture the loaded header for a wave after it has fully settled (truth).
    task truth(input [8:0] wv);
        rst_n=0; repeat(8) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
        write_reg(8'h20, {7'd0, wv[8]}); write_reg(8'h08, wv[7:0]);
        wait (dbg_slot0_hdr_start != 22'd0); repeat(3000) @(posedge clk);
        hs=dbg_slot0_hdr_start; hl=dbg_slot0_hdr_loop; he=dbg_slot0_hdr_end; hb=dbg_slot0_hdr_bits; har=dbg_slot0_ar;
    ...
    initial begin
        ...
        rst_n=0; repeat(8) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
        set_and_key(9'd122, 60, "setup122");
        set_and_key(9'd123, 2,  "122->123 gap2");
        check("122->123 gap2: header is wave123's start", dbg_slot0_hdr_start == hs);
        check("122->123 gap2: header is wave123's bits",  dbg_slot0_hdr_bits  == hb);
        // 124 -> 123
        rst_n=0; repeat(8) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
        set_and_key(9'd124, 60, "setup124");
        set_and_key(9'd123, 2,  "124->123 gap2");
        check("124->123 gap2: header is wave123's start", dbg_slot0_hdr_start == hs);
        // Longer gap should always be correct.
        rst_n=0; repeat(8) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
        set_and_key(9'd122, 60, "setup122b");
        set_and_key(9'd123, 30, "122->123 gap30");
        check("122->123 gap30: header is wave123's start", dbg_slot0_hdr_start == hs);
        $display("\n=== %0d PASS, %0d FAIL ===", passes, fails);
        if (fails==0) $display("*** ALL TESTS PASSED ***");
        else          $display("!!! %0d FAILS (stale-header race reproduced) !!!", fails);
        $finish;
    end

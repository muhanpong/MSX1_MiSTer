...
module tb_rekey;
    ...
    // ── Re-key cycle: replicate the BASIC tester loop ─────────────────────────
    //   key off (0x68=0) → short gap → key on (0x68=0x80), WITHOUT a full reset.
    // The bug: pos/stepPtr reset on every key-on edge, but env restart was gated
    // on cur_state==EG_OFF, so a re-key while the slot is still EG_REL (or SUS/DEC)
    // restarts the sample at its decayed/zero volume → silent "toggle".  Reference
    // keyOnHelper restarts env on EVERY key-on.
    task rekey(input int gap_frames, input int hold_frames, input string label);
        logic [2:0] pre_state; logic [9:0] pre_vol;
        write_reg(8'h68, 8'h00);             // KEY OFF
        repeat (gap_frames) frames(1);
        pre_state = dbg_slot0_dyn_env_state;  // state seen at the re-key edge
        pre_vol   = dbg_slot0_dyn_env_vol;
        write_reg(8'h68, 8'h80);             // KEY ON (re-trigger)
        frames(2);                            // let the edge propagate + attack
        reset_mon(); frames(hold_frames);     // measure the held note
        $display("  [%s] pre(state=%0d vol=%h) -> post(state=%0d vol=%h) pcm_nz=%0d/%0d",
                 label, pre_state, pre_vol,
                 dbg_slot0_dyn_env_state, dbg_slot0_dyn_env_vol, pcm_nz_recent, pcm_cnt);
        check($sformatf("%s: note alive after re-key (pre_state=%0d)", label, pre_state),
              pcm_nz_recent > 0 && dbg_slot0_dyn_env_state != EG_OFF);
    endtask

    initial begin
        // Replicate the BASIC tester on wave 3: set wave, key on, HOLD (INPUT$
        // wait), key off, short gap, re-key — repeatedly.  Each re-key must
        // re-attack (like openMSX "replay"), not toggle on/off.
        rst_n=0; repeat(8) @(posedge clk); rst_n=1; repeat(4) @(posedge clk);
        write_reg(8'h68, 8'h00); write_reg(8'h20, 8'h00); write_reg(8'h08, 8'd3);
        wait (dbg_slot0_hdr_bits != 2'b00 || dbg_slot0_hdr_start != 22'd0);
        repeat (2000) @(posedge clk);
        write_reg(8'h68, 8'h80);              // first KEY ON
        frames(2); reset_mon(); frames(30);
        $display("  [first] state=%0d vol=%h pcm_nz=%0d/%0d  hdr start=%h AR=%0d",
                 dbg_slot0_dyn_env_state, dbg_slot0_dyn_env_vol, pcm_nz_recent, pcm_cnt,
                 dbg_slot0_hdr_start, dbg_slot0_ar);
        check("first key-on: note alive", pcm_nz_recent > 0);

        // Several BASIC-style re-key cycles with varying gap (lands in EG_REL).
        rekey(1,  30, "rekey gap=1");
        rekey(2,  30, "rekey gap=2");
        rekey(4,  30, "rekey gap=4");
        rekey(1,  30, "rekey gap=1 b");

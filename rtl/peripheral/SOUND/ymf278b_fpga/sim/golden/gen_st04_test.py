#!/usr/bin/env python3
"""
Replays ST04O.VGM (Strikers 1945 II, OPL4 arcade rip) through the PCM golden
...
Outputs: mem4.bin / mem4.hex (4MB) and sc_st04.txt.
"""
import struct, os

VGM = "/home/muhanpong/Documents/github/MSX1_MiSTer/vgm_test/sdcard/ST04O.VGM"
WAIT_CAP = 8
...
frames = frame + 200    # tail room for release
open(f"{out}/mem4.bin", "wb").write(MEM)
with open(f"{out}/mem4.hex", "w") as f:
    for b in MEM:
        f.write(f"{b:02x}\n")
with open(f"{out}/sc_st04.txt", "w") as f:
    f.write("\n".join(lines) + "\n")
print(f"sc_st04.txt: {len(lines)} writes ({nmapped} wave-mapped), {frames} frames")

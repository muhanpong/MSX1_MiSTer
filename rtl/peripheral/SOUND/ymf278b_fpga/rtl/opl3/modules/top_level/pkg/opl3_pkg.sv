...
package opl3_pkg;
     ...
     * domain crossings.
     */
    localparam CLK_FREQ = 14.318182e6; // MSX MiSTer: clk_sdram/6 = 85909090/6
    localparam DAC_OUTPUT_WIDTH = 24;
    localparam INSTANTIATE_TIMERS = 1; // 1 = instantiate OPL3 timers + trick_sw_detection.
                                       // REQUIRED for chip detection: MoonSound software
                                       // (e.g. MBwave) starts a timer and polls the status
                                       // overflow bits.  With this at 0 the `status` net has
                                       // no driver (stuck 0) and detection always fails.

...
(
    ...
    output logic irq_n,
    // Expose the internal OPL3 status byte (IRQ + timer1/timer2 overflow
    // flags) so the host wrapper can return it on a status-port read.  Real
    // software (e.g. MoonSound players) detects the chip by starting a timer
    // and polling these bits; leaving them at 0 breaks detection.
    ...
    // Surface the internal status register (driven by host_if/timers via .*).

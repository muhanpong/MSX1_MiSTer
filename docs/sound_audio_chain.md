...
행 번호는 `MSX1_20260904a_audio` 빌드 기준.
...
| # | 위치 | 동작 | 들어오는 폭 | 나가는 폭 |
|---|---|---|---|---|
| ① | `msx_slots.sv:129` | ±32767 하드 클램프 | 19b signed | 16b `cart_sound` |
...
| 음원 | 사다리 | 폭 | entry 0 | Off | 정의 |
|---|---|---|---|---|---|
| 내장 PSG | `psg_mul` | `[8:0]` unsigned | 128 · 0 dB | entry 10 | `msx.sv:198` |
| OPLL (MSX-MUSIC) | `vol_mul` | `[9:0]` signed | 128 · 0 dB | entry 10 | `msx_slots.sv:103` |
| SCC (A+B 공용) | `vol_mul` | `[9:0]` signed | 128 · 0 dB | entry 10 | `msx_slots.sv:103` |
| OPL4 PCM | `pcm_post` | `[11:0]` | 128 · 0 dB | 뮤트 별도 | `ymf278b_top.sv:371` |
| OPL4 FM | `fm_gain` | `[11:0]` | **203 · +4 dB** | 뮤트 별도 | `ymf278b_top.sv:390` |
| 카트리지 PSG | — | — | — | **없음** | `msx_slots.sv:685` |
...
**② PSG 트림은 세 신호를 한꺼번에 잡는다.**
`audioPSG` = PSG 채널 + 키보드 비프(PPI 포트 C 비트 7, `msx.sv:544`) + 테이프 입력.
실기도 이 셋이 한 믹서로 가므로 트림이 공용인 건 맞다. 다만 "Off"만은 `ay_ch_mix`만
끊는다 — 배율을 0으로 만들면 비프와 테이프까지 죽기 때문이다.

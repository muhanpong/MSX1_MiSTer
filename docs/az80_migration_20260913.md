...
### 실기 1차: 부팅 실패 (20260914, `20260914a_dual`, CFG 속도=0 → A-Z80 경로)
증상: BIOS 미도달, 화면은 PCM 디버그 오버레이 스트립만(VDP 미초기화). T80s 경로(21.5) 미시험.
- 가설1 **M1 wait pair(74LS74) 위상 라이브락** — 3.58에선 cpu_turbo=0이라 wait_n===wait_m1_n인데
  이 회로는 어떤 A-Z80 벤치에도 없었음. 새 벤치 `tb_az80_m1wait.sv`(ce 위상 전수 스윕): **기각** —
  전부 정지. 단 위상 절반에서 M1 wait가 **누락**(225클럭 vs 253) = MSX2 M1 웨이트가 A-Z80에서 위상
  의존. 정확도 결함(후속: 쌍을 az80_clk로 옮기거나 az80 도메인에서 생성), 부팅 원인 아님.
- 가설2 리셋 중 az80_clk 정지(clkgen) → A-Z80 내부 리셋체인 미전파: 브링업 벤치도 같은 조건으로
  통과하므로 약함. 미배제.
- 다음 분기: OSD 21.5(T80s)로 부팅되면 A-Z80 통합 문제, 안 되면 듀얼 공통 인프라(mux/리셋) 문제.

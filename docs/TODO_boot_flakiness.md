# TODO — 부팅 간헐 실패 조사 (**보류**, 2026-08-19 관측 / 2026-08-23 보류)

> **PARKED 2026-08-23, 사용자 결정.** 증상이 **슬롯 B + Yamanooto 조합에서만**
> 나타나는 것으로 정리되어, `docs/TODO_yamanooto.md` 항목 6(슬롯 B Yamanooto
> 부팅률 50% 미만)과 **같은 뿌리**로 보고 함께 보류한다. 어느 한쪽을 재개하면
> 다른 쪽도 같이 봐야 한다.
>
> 그 사이 이 문서의 **1순위 가설이었던 DDR3 스테이징 침범은 실제로 수정되었다**
> (슬롯 B `0x1100000` -> `0x3000000`, `20260823c` 이후). 재개 시 첫 단계는
> 아래 확인항목이 아니라 **그 수정 전후 A/B**다 — `20260822b_yamawrite`(재배치 전)
> vs 현재 빌드에 동일 롬을 슬롯 B로 올려 발생률을 비교하면 가설이 바로 갈린다.

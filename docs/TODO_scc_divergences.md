...
**상태: ◆◆CLOSED** — sim 검증 + 뮤테이션 증명 + **실기 확인 2026-08-30**
(`MSX1_20260830b_devclear`, 도구 `tools/sccch5/`).

| 디스크 | 3초(창 OPEN) | 6초(창 CLOSED) | 결과 |
|---|---|---|---|
| `SCCCH5.DSK` | 드론+높은톤 | **둘 다 계속** | ch5가 자기 파형 유지 = 수정 유효 |
| `SCCCH5N.DSK` (대조군) | 드론+높은톤 | **드론만** | 칩을 진짜 Compatible로 내리면 ch5가 ch4(무음) 미러 |

대조군이 **다르게** 나왔으므로 측정에 판별력이 있는 상태에서 얻은 결과다.
판정을 음색 비교가 아니라 **소리 유무**로 만든 게 요점(ch4 파형=무음, ch5=구형파,
ch1=한 옥타브 아래 드론으로 "아직 돌고 있다"를 표시).
